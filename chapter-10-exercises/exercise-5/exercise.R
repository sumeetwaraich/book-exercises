...
most_popular_in_year <- function(year) {
  ...
  most_popular <- year_names[[year_names$prop == max(year_names$prop), "name"]]
  return(most_popular)
}
...
number_in_million <- function(name, year) {
  popularity_name <- names[names$year == year & names$name == name, "prop"]
  round(popularity_name * 1000000, 1)
}
